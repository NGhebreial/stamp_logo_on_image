...
class ImageSave {
  ...
  Future<String> saveImage(File image) async {
    ...
    if(Platform.isAndroid){
      return await ImagePickerSaver.saveFile(fileData: image.readAsBytesSync(), title: "LogoOnImage_"+now+".png",
          description: "LogoOnImage_"+now+".png", directory: "LogoOnImagePictures");
    }
    else{
  ...
  Future<File> saveImageTemporal(File image, String temporalDir) async {
    ...
    var file = File('$temporalDir/'+_generateUuidJpg().toString())..writeAsBytesSync(image.readAsBytesSync());
  ...
  File saveLogo(File image, String logosDir) {
    ...
    var file = File('$logosDir/'+_generateUuidJpg().toString())..writeAsBytesSync(image.readAsBytesSync());
